...
def _claude_verify_sprite(pil_image, expected_subject: str, role: str) -> dict:
    ...
    prompt = (
        ...
        f" - Look at the HEAD/FACE specifically — where does the NOSE point?\n"
        f" - 'right' = strict side profile, ONE ear visible on the left side of the head, nose pointing to the right edge of the image, AND the body/feet point in the SAME direction as the head.\n"
        f" - 'left'  = strict side profile, ONE ear visible on the right side of the head, nose pointing to the left edge of the image, AND the body/feet point in the SAME direction as the head.\n"
        ...
        f" - 'unclear' — USE THIS in any of these cases:\n"
        f"     * You're not highly confident which direction the face points.\n"
        f"     * The head and body point in DIFFERENT directions (e.g. running right while looking back over the shoulder to the left — common in 'escape' artwork but unusable in a side-scroller).\n"
        f"     * 3/4 view (slightly turned, neither full profile nor full front).\n"
        f"For vehicles, 'facing' is where the FRONT of the vehicle points (headlights/grille).\n"
